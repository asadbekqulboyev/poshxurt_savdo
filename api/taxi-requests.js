...
const mapReq = (row) => ({
  ...
  price: row.price,
  phone: row.phone,
  fromLat: row.from_lat ?? undefined,
  fromLng: row.from_lng ?? undefined,
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const b = req.body || {};
    if (!b.from || !b.to)
      return res.status(400).json({ error: "from va to shart" });
    ...
    const { error } = await supabaseAdmin.from("taxi_requests").insert([
      {
        ...
        phone: dbUser.phone,
        from_lat: typeof b.fromLat === "number" ? b.fromLat : null,
        from_lng: typeof b.fromLng === "number" ? b.fromLng : null,
      },
    ]);
    if (error) return res.status(500).json({ error: error.message });
