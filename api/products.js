...
import { supabaseAdmin, envMissing } from "./_lib/supabaseAdmin.js";
...
export default async function handler(req, res) {
  ...
  if (envMissing) {
    return res
      ...
      .json({
        error:
          "Server sozlanmagan: SUPABASE env yo`q. Vercel`ga env qo`shib Redeploy qiling.",
      });
  }
  ...
  if (req.method === "POST") {
    ...
    if (!tgUser)
      return res.status(401).json({ error: "Telegram imzosi tasdiqlanmadi" });
    ...
    const p = req.body || {};
    if (!p.title || !p.price)
      return res.status(400).json({ error: "title va price shart" });
  ...
  if (req.method === "DELETE") {
    ...
    if (!Array.isArray(ids) || ids.length === 0)
      return res.status(400).json({ error: "ids kerak" });
    ...
    const paths = urls
      .map((u) => {
        const m = u.indexOf("/object/public/product-images/");
        return m === -1
          ? null
          : u.slice(m + "/object/public/product-images/".length);
      })
      .filter(Boolean);
    if (paths.length)
